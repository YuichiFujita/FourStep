...
public:
	...
	// コンストラクタ
	CObject();
	explicit CObject(const ELabel label, const int nPriority = object::DEFAULT_PRIO);
...
private:
	...
	// 静的メンバ変数
	static CObject	*m_apTop[object::MAX_PRIO];	// 先頭のオブジェクトへのポインタ
	static CObject	*m_apCur[object::MAX_PRIO];	// 最後尾のオブジェクトへのポインタ
