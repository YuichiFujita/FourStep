...
// メモリ開放マクロ
#define SAFE_REF_RELEASE(p)	if ((p) != nullptr) { if (FAILED((p)->Release((p)))) { assert(false); return E_FAIL; } }	// 参照ポインタ付きRelease関数の破棄マクロ
...
#define GET_RETENTION	(CManager::GetInstance()->GetRetentionManager())		// データ保存情報取得
